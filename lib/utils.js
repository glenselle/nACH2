...
function generateString(object, cb) {
	var counter = 1;
	var result = '';
	var objectCount = Object.values(object).length;
...
var isBusinessDay = module.exports.isBusinessDay = function(day) {
	var d = luxon.DateTime.fromJSDate(day);
	return d.weekday >= 1 && d.weekday <= 5; // 1=Monday, 5=Friday
};
...
var computeBusinessDay = module.exports.computeBusinessDay = function(businessDays) {
  var luxonDate = arguments[1] ? luxon.DateTime.fromJSDate(arguments[1]) : luxon.DateTime.local();
  var counter = 0;

  function addDay(date) {
    var nextDay = date.plus({ days: 1 });
    if (isBusinessDay(nextDay.toJSDate())) {
      counter++;
    }

    return (counter === businessDays) ? nextDay.toJSDate() : addDay(nextDay);
  }

  return addDay(luxonDate);
};
