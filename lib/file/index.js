...
function File(options) {
	...
	this.header = (options && options.header) ? _.merge(options.header, require('./header')(), _.defaults) : require('./header')();
	this.control = (options && options.control) ? _.merge(options.control, require('./control'), _.defaults) : _.cloneDeep(require('./control'));
    ...
    if(options && options.immediateDestination) {
        this.header.immediateDestination.value = utils.computeCheckDigit(options.immediateDestination);
    }

	this._batchSequenceNumber = (options && options.batchSequenceNumber) ? Number(options.batchSequenceNumber) : 0
