...
function validateRequiredFields(object) {
	Object.values(object).forEach((field) => {
		...
		if(field.required === true && (Number.isNaN(field.value) || field.value === null || field.value === undefined || field.value.toString().length === 0)) {
			throw new nACHError({
...
function validateACHAddendaTypeCode(addendaTypeCode) {
	if(addendaTypeCode.length !== 2 || !ACHAddendaTypeCodes.includes(addendaTypeCode)) {
		throw new nACHError({
...
function validateACHCode(transactionCode) {
	if(transactionCode.length !== 2 || !ACHTransactionCodes.includes(transactionCode)) {
		throw new nACHError({
...
function validateACHServiceClassCode(serviceClassCode) {
	if(serviceClassCode.length !== 3 || !ACHServiceClassCodes.includes(serviceClassCode)) {
		throw new nACHError({
