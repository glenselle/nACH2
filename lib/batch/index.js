...
Batch.prototype.generateHeader = function() {
  return utils.generateString(this.header)
}

Batch.prototype.generateControl = function() {
  return utils.generateString(this.control)
}

Batch.prototype.generateEntries = function() {
  return this._entries.map(function(entry, done) {
    return entry.generateString()
  }).join(utils.newLineChar())
}

Batch.prototype.generateString = function() {
  var headerString = this.generateHeader()
  var entryString = self.generateEntries()
  var controlString =  self.generateControl()

  return headerString + utils.newLineChar() + entryString  + controlString
}
